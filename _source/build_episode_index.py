...
def build(log=print, cancel_evt=None) -> Path:
    ...
    for arc in arcs:
        ...
        for ep in ep_nums:
            ...
            # Sort sources for stable UI: onepace > muhn > nyaa, with the
            # official Galaxy9000 uploader winning ties inside the Nyaa group.
            sources.sort(key=lambda s: (
                {"onepace": 0, "muhn": 1, "nyaa": 2}.get(s["kind"], 9),
    ...
    # Enrich with SpykerNZ canonical titles + plots. Season N (Spyker) =
    # canonical arc index N-1. Best-effort: a failed fetch is logged and we
    # ship the index without canonical titles (the UI falls back to the
    # auto-generated "{arc} {NN}" form).
    try:
